/* global React, TPL, Icon, L, cx, DevicePreview, SegmentMeter, VersionHistory, StatusPill, ChannelTag, KebabMenu, DeleteModal, NeToast, ArchiveModal, deleteMode, ApprovalTimeline, historyFromVersions, SubmitModal, apfmt */
/* ============================================================
   ...
   ============================================================ */
const { useState: useVS, useMemo: useVM, useRef: useVR, useEffect: useVE } = React;
...
function ReadBox({ label, icon, children, subj }) {
  return (
    <div>
...
function ChannelFields({ tpl, ver, lang, t }) {
...
function ContentStructure({ tpl, ver, lang, t, mode }) {
...
function VariablesUsed({ ver, lang, t }) {
  const tokens = useVM(() => {
...
function VersionSwitcher({ versions, idx, onPick, lang, t }) {
...
function AttrOverview({ tpl, ver, lang, t }) {
...
/* ---------- lifecycle / audit ---------- */
function Lifecycle({ tpl, ver, lang, t, archivedMeta }) {
  const Row = ({ k, children }) => <div className="ajb-kv__row"><span className="ajb-kv__key">{k}</span><span className="ajb-kv__val">{children}</span></div>;
...
/* ============================================================
   VIEW DETAILS — layout
   ============================================================ */
function ViewDetails({ tpl, idx, onPick, lang, t, editHref, backHref }) {
  const ver = tpl.versions[idx];
  const latest = tpl.versions.find(v => v.current) || tpl.versions[0];
  const [sample, setSample] = useVS(false);
  const [clang, setClang] = useVS(lang);
  const [del, setDel] = useVS(null);
  const [arc, setArc] = useVS(null);
  const [toast, setToast] = useVS(null);
  const [deleted, setDeleted] = useVS(false);
  const [archivedMeta, setArchivedMeta] = useVS(null);
  const [submitOpen, setSubmitOpen] = useVS(false);
  const [submittedTo, setSubmittedTo] = useVS(null);

  useVE(() => { setDel(null); setArc(null); setToast(null); setDeleted(false); setArchivedMeta(null); setClang(lang); setSubmitOpen(false); setSubmittedTo(null); }, [tpl.id]);
  useVE(() => {
    if (!toast) return;
        ...
        <div className="ts-tablewrap">
          <div className="tl-empty">
            <div className="ajb-empty">
              ...
              <a href={backHref} className="ajb-btn ajb-btn--sand"><Icon name="arrow-left" />{t.delGoneBack}</a>
            </div>
          </div>
        </div>
        ...
        <div className="ts-actions">
          ...
          {archived
            ...
            : inReview
              ...
              : isDraft
                ? <React.Fragment>
                    <a href={editHref} className="ajb-btn ajb-btn--secondary"><Icon name="pencil" />{t.editTpl}</a>
                    <button type="button" className="ajb-btn ajb-btn--sand" onClick={() => setSubmitOpen(true)}><Icon name="send" />{t.apSubmitForReview}</button>
                  </React.Fragment>
                : <a href={editHref} className="ajb-btn ajb-btn--sand"><Icon name="pencil" />{t.editTpl}</a>}
        </div>
      </header>
...
Object.assign(window, { ViewDetails, TokenText, VariablesUsed, VersionSwitcher });
