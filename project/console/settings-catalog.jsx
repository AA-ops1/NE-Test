/* global React, TPL, Icon, L, cx, DropSelect */
/* ============================================================
...
function catalogUsage(tab, item) {
  const list = (window.TPL && TPL.LIST) || [];
  ...
  if (tab === 'variables') {
    ...
    (window.TPL && TPL.SAMPLE ? [TPL.SAMPLE.content.en, TPL.SAMPLE.content.ar] : []).forEach(c => { if (re.test(c)) n++; });
    return n;
  }
  return 0;
}
...
/* ---------- field schema ---------- */
function fieldsFor(tab, s, groups) {
...
function useReorder(setItems) {
  const from = useCR(null);
  return {
...
function GroupChip({ gid, groups, lang }) {
...
/* ---------- TABLE direction ---------- */
function CatalogTable({ tab, items, groups, s, lang, canReorder, onEdit, onDelete }) {
...
/* ---------- CARDS direction ---------- */
function CatalogCards({ tab, items, groups, s, lang, canReorder, onEdit, onDelete }) {
  const ro = useReorder(onEdit.reorder);
  const isVar = tab === 'variables';
  return (
...
function CatalogEditor({ surface, tab, item, isNew, groups, s, lang, onSave, onCancel }) {
  const [draft, setDraft] = useCS(item);
  useCE(() => { setDraft(item); }, [item]);
...
/* ---------- delete confirm ---------- */
function CatalogDelete({ tab, item, s, lang, onCancel, onConfirm }) {
...
Object.assign(window, { CatalogTable, CatalogCards, CatalogEditor, CatalogDelete, fieldsFor, blankItem, catalogUsage });
