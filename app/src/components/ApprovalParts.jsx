...
import { L, cx, StatusPill } from './TemplateParts';

export function apfmt(str, map) { return String(str).replace(/\{(\w+)\}/g, (m, k) => (k in map ? map[k] : m)); }
const AP = () => TPL.AP;

/* ---------- avatar / person ---------- */
export function Avatar({ p, size = 'sm' }) {
  return <span className={`ajb-avatar ajb-avatar--${size}`}><span>{p ? p.init : '?'}</span></span>;
}

export function PersonChip({ p, lang, sub }) {
  return (
...
export function PriorityTag({ id, lang }) {
...
export function withTokens(str, keyPfx) {
...
export function FieldDiff({ label, before, after, lang }) {
...
export function changedFields(sub, lang, t) {
...
/* ============================================================
   ...
   ============================================================ */
export function ApprovalTimeline({ events, lang, awaiting }) {
  const t = TPL.T[lang];
  const EV = AP().EVENTS;
...
export function historyFromVersions(tpl, lang) {
  const events = [];
...
/* ============================================================
   ...
   ============================================================ */
export function DecisionBar({ lang, t, blocked, variant, onApprove, onSendBack, onReject, onReassign }) {
  const [mode, setMode] = useState(null);      // null | 'note' | 'send' | 'reject'
...
/* ============================================================
   ...
   ============================================================ */
export function ReassignModal({ name, lang, t, onClose, onConfirm }) {
...
/* ============================================================
   ...
   ============================================================ */
export function SubmitModal({ name, lang, t, onClose, onConfirm }) {
  const [pick, setPick] = useState(null);
  const [note, setNote] = useState('');
  const stop = e => e.stopPropagation();
  const dir = lang === 'ar' ? 'rtl' : 'ltr';
